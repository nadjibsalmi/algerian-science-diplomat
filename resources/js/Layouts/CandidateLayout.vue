...
const nav = [
    { label: 'Tableau de bord', href: route('candidate.dashboard') },
    { label: 'Profil', href: route('candidate.profile') },
    { label: 'Candidatures', href: route('candidate.applications.index') },
    { label: 'Documents', href: route('documents.index') },
    { label: 'Favoris', href: route('candidate.favorites.index') },
    { label: 'Messages', href: route('messaging.conversations.index') },
    { label: 'Notifications', href: route('notifications.index') },
    { label: 'Alertes', href: route('notifications.alerts.index') },
    { label: 'Sessions', href: route('sessions.index') },
];
</script>
    ...
    <div class="flex min-h-screen bg-[#F4F6F8]">
        ...
        >
            <div class="flex h-16 items-center gap-3 border-b border-gray-100 px-5">
                ...
                <span class="rounded-full bg-[#006233]/10 px-2 py-0.5 text-xs font-medium text-[#006233]">
                    Candidat
                </span>
            </div>

            <nav class="flex flex-col gap-0.5 p-3" aria-label="Navigation candidat">
                <Link
                    v-for="item in nav"
                    :key="item.href"
                    :href="item.href"
            ...
            <header class="flex h-16 items-center gap-4 border-b border-gray-200 bg-white px-4 lg:px-6">
                ...
                <div class="ms-auto flex items-center gap-3">
                    <Link
                        ...
                        class="text-sm text-gray-500 hover:text-[#0B1F3A]"
                    >
                        Offres
                    </Link>
                </div>
